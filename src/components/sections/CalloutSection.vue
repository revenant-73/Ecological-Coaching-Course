<template>
  <div class="group relative animate-fade-in-up">
    ...
    <div class="relative bg-gray-800 border border-accent-orange group-hover:border-accent-orange backdrop-blur-sm p-6 sm:p-8 rounded-xl transition-all duration-500">
      ...
      <div class="text-gray-200 text-lg sm:text-xl whitespace-pre-wrap leading-relaxed space-y-3">
  ...
  return text
    ...
    .map(line => {
      line = line
        ...
        .replace(/>(.*?)$/gm, '<blockquote class="border-l-2 border-accent-orange pl-3 my-3 text-gray-300 italic">$1</blockquote>')
